...
class trackingEvaluation(object):
    ...
    def __init__(self,
        ...
        self.n_frames = []
        self.sequence_name = []
        with open(gt_map, "r") as fh:
            for i, l in enumerate(fh):
                fields = l.split(" ")
                self.sequence_name.append(fields[0])
        ...
        self.t_sha = res_folder
        self.t_path = res_folder
        ...
        self.gt_tracker_trajectories = [[] for x in range(self.n_sequences)]
    ...
    def _loadData(self,
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            for line in f:
                ...
                t_data.truncation = int(float(
                    fields[3]))  # truncation [-1,0,1,2]
                t_data.occlusion = int(float(
                    fields[4]))  # occlusion  [-1,0,1,2]
                ...
                if not loading_groundtruth and eval_2d is True and (
                        t_data.x1 == -1 or t_data.x2 == -1 or t_data.y1 == -1
                        or t_data.y2 == -1):
                    eval_2d = False
                if not loading_groundtruth and eval_3d is True and (
                        t_data.X == -1000 or t_data.Y == -1000
                        or t_data.Z == -1000):
                    eval_3d = False
    ...
    def compute3rdPartyMetrics(self):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            seq_tracker_trajectories = defaultdict(list)
            ...
            for f in range(len(seq_gt)):
                ...
                for gg in g:
                    ...
                    seq_tracker_trajectories[gg.track_id].append(None)
                ...
                for row, col in association_matrix:
                    ...
                    if c < max_cost:
                        ...
                        seq_tracker_trajectories[g[row].track_id][-1] = t[col]
            ...
            self.gt_tracker_trajectories[seq_idx] = seq_tracker_trajectories
        ...
        for seq_idx, (seq_trajectories, seq_ignored, seq_tracker_trajectories) in enumerate(
                zip(self.gt_trajectories, self.ign_trajectories, self.gt_tracker_trajectories)):
            if len(seq_trajectories) == 0:
                continue
            tmpMT, tmpML, tmpPT, tmpId_switches, tmpFragments = [0] * 5
            n_ignored_tr = 0
            for g, ign_g, t in zip(seq_trajectories.values(),
                                   seq_ignored.values(),
                                   seq_tracker_trajectories.values()):
    ...
    def analyze(self):
        fp, fn, idsw = 0, 0, 0
        filename = os.path.join(self.t_sha, "analysis_%s.txt" % self.cls)
        print(f'\nAnalysis Sequence Breakdown writing to {filename}')
        with open(filename, "w+") as dump:
            for seq_idx in range(len(self.groundtruth)):
                seq_fp, seq_fn, seq_idsw = 0, 0, 0
                seq_gt = self.groundtruth[seq_idx]
                seq_tracker = self.tracker[seq_idx]
                ...
                for f in range(len(seq_gt)):
                    g = seq_gt[f]
                    t = seq_tracker[f]
                    img_path = self.gt_path.replace('label', 'image')
                    seq_name = self.sequence_name[seq_idx]
                    out_path = osp.join(self.t_path, '../analysis', self.cls, seq_name)
                    os.makedirs(out_path, exist_ok=True)
                    img = cv2.imread(osp.join(img_path, seq_name,
                                     '{}.png').format(str(f).zfill(6)))
                    save = False  # only save if the image has FP/FN/IDSW

                    for gg in g:
                        if gg.tracker == -1 and not gg.ignored:
                            # fn: yellow (0, 255, 255)
                            color = (0, 255, 255)
                            cv2.rectangle(img, (int(gg.x1), int(gg.y1)), (int(gg.x2), int(gg.y2)),
                                          color, thickness=2)
                            save = True
                            seq_fn += 1
                    for tt in t:
                        if not tt.valid and not tt.ignored:
                            # fp: red (0, 0, 255)
                            color = (0, 0, 255)
                            cv2.rectangle(img, (int(tt.x1), int(tt.y1)), (int(tt.x2), int(tt.y2)),
                                          color, thickness=2)
                            save = True
                            seq_fp += 1
                        if tt.id_switch:
                            ...
                            cv2.rectangle(img, (int(tt.x1), int(tt.y1)), (int(tt.x2), int(tt.y2)),
                                          color, thickness=2)
                    if save:
                        file_name = osp.join(out_path, '{}.png'.format(str(f).zfill(6)))
                        cv2.imwrite(file_name, img)
                summary = 'Sequence: {}, FP: {:4d}, FN: {:4d}, IDSW: {:4d}\n'.format(seq_name, seq_fp, seq_fn, seq_idsw)
                dump.write(summary)
                fp += seq_fp
                fn += seq_fn
                idsw += seq_idsw
            summary = 'Total FP: {:4d}, Total FN: {:4d}, Total IDSW: {:4d}\n'.format(fp, fn, idsw)
            dump.write(summary)


def evaluate(gt_map, gt_folder, res_folder, analyze=False):
    # start evaluation and instanciated eval object
    print('Evaluating KITTI results...')
    classes = []
    for c in ("car", "pedestrian"):
        e = trackingEvaluation(
            gt_map=gt_map, gt_folder=gt_folder, res_folder=res_folder, cls=c)
        # load tracker data and check provided classes
        try:
            if not e.loadTracker():
                print(f"Tracking data of class {c} cannot be found in {res_folder}")
                continue
            classes.append(c)
        except:
            break
...
def parse_args():
    ...
    parser.add_argument('--analyze', action='store_true', help='save analyze results')
...
if __name__ == "__main__":
    ...
    success = evaluate(args.gt_map, args.gt_folder, args.res_folder, args.analyze)
