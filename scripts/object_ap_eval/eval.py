...
def get_official_eval_result(gt_annos,
    ...
    """
    # During the evaluation, it will loop through all difficulties
    # The three threshold lists are for "bbox", "bev", "3d"
    # The inner list is for different classes
    overlap_official_70 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                                    [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                                    [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7]])
    overlap_loose3dt_50 = np.array(
        [[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5, 0.5],
         [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5],
         [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5]])
    min_overlaps = np.stack([overlap_official_70, overlap_loose3dt_50],
                            axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
...
def get_coco_eval_result(gt_annos,
    ...
    for curcls in current_classes:
        if isinstance(curcls, str):
            ...
            current_classes_int.append(curcls)
    current_classes = current_classes_int
    overlap_ranges = np.zeros([3, 3, len(current_classes)], dtype=object)
    for i, curcls in enumerate(current_classes):
        overlap_ranges[:, :, i] = np.array(
            class_to_range[curcls], dtype=object)[:, np.newaxis]
    result = ''
    # check whether alpha is valid
    compute_aos = False
    for anno in dt_annos:
