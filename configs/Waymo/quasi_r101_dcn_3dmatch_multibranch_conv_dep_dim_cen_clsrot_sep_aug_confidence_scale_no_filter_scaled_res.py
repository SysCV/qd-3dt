...
test_cfg = dict(
    analyze=False,
    show=False,
    save_txt=dict(pedestrian=0, cyclist=1, car=2, dontcare=3),
    use_3d_center=True,
    rpn=dict(
        nms_across_levels=False,
        nms_pre=1000,
    ...
    track=dict(
        ...
        lstm_name='VeloLSTM',
        lstm_ckpt_name=
        './checkpoints/batch128_min10_seq10_dim7_VeloLSTM_waymo_100_linear.pth',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=dict(VID=data_root +
                      'anns/tracking_train.json'),
        img_prefix=dict(VID=data_root + 'images/'),
        img_scale=(1280, 854),
