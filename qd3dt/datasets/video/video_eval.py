...
class VideoEval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        ...
        self.evalVids = defaultdict(
            list)  # per-video per-category evaluation results [KxAxV] elements
    ...
    def _prepare(self, with_occlusion=False):
        ...
        for vidId in p.vidIds:
            if p.useCats:
                gt = self.cocoGt.loadAnns(
                    self.cocoGt.getAnnIds(vidIds=[vidId], catIds=p.catIds))
                dt = self.cocoDt.loadAnns(
                    self.cocoDt.getAnnIds(vidIds=[vidId], catIds=p.catIds))
            else:
                gt = self.cocoGt.loadAnns(
                    self.cocoGt.getAnnIds(vidIds=[vidId]))
                dt = self.cocoDt.loadAnns(
                    self.cocoDt.getAnnIds(vidIds=[vidId]))
            imgIds = self.cocoGt.getImgIdsFromVidId(vidId)
            gts += self.anns2Tracks(
                gt, imgIds, vidId, with_occlusion=with_occlusion)
            dts += self.anns2Tracks(
                dt, imgIds, vidId, with_occlusion=with_occlusion)
        ...
        self.evalVids = defaultdict(
            list)  # per-video per-category evaluation results
        self.eval = {}  # accumulated evaluation results
    ...
    def evaluate(self, with_occlusion=False, vidIds=None):
        ...
        print('Matching...')
        self.evalVids = [
            evaluateVid(vidId, catId, areaRng, tempRng, speedRng, maxDet)
            for catId in catIds for areaRng in p.areaRng
            for tempRng in p.tempRng for speedRng in p.speedRng
            for vidId in p.vidIds
        ]
    ...
    def anns2Tracks(self, anns, img_ids, vid_id, with_occlusion=True):
        ...
        for i in tracks.keys():
            ...
            tracks[i]['meanConsecutiveIoU'] = np.mean(
                tracks[i]['meanConsecutiveIoU']) if len(
                    tracks[i]['meanConsecutiveIoU']) > 0 else -1
    ...
    def computeIoU(self, vidId, catId):
        ...
        for i, j in np.ndindex(ious.shape):
            ious[i,
                 j], det_ious[i,
                              j] = self.avgOverlapAndDetOverlap(dt[i], gt[j])
    ...
    def evaluateVid(self, vidId, catId, aRng, tempRng, speedRng, maxDet):
        ...
        iscrowd = [int(o['iscrowd']) for o in gt]
        # load computed ious
        ious, det_ious = self.ious[vidId, catId][0][:, gtind], self.ious[
            vidId,
            catId][1][:,
                      gtind] if len(self.ious[vidId,
                                              catId]) > 0 else self.ious[vidId,
                                                                         catId]
        T = len(p.iouThrs)
        G = len(gt)
        D = len(dt)
    ...
    def accumulate(self, p=None):
        ...
        M = len(p.maxDets)
        precision = -np.ones((T, R, K, A, TEMP, SPEED,
                              M))  # -1 for the precision of absent categories
        ...
        temp_list = [
            n for n, a in enumerate(map(lambda x: tuple(x), p.tempRng))
            if a in setTEMP
        ]
        speed_list = [
            n for n, a in enumerate(map(lambda x: tuple(x), p.speedRng))
            if a in setSPEED
        ]
                ...
                for temp, temp0 in enumerate(temp_list):
                    ...
                    for speed, speed0 in enumerate(speed_list):
                        ...
                        for m, maxDet in enumerate(m_list):
                            E = [
                                self.evalVids[Nk + Na + Ntemp + Nspeed + i]
                                for i in i_list
                            ]
                            ...
                            tp_sum = np.cumsum(
                                tps, axis=1).astype(dtype=np.float)
                            fp_sum = np.cumsum(
                                fps, axis=1).astype(dtype=np.float)
                            for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                                ...
                                try:
                                    ...
                                    pass
                                precision[t, :, k, a, temp, speed,
                                          m] = np.array(q)
                                scores[t, :, k, a, temp, speed,
                                       m] = np.array(ss)
        self.eval = {
            'params': p,
    ...
    def summarize(self, by_cat=True):
        ...
        def _summarize(ap=1,
            ...
            tempind = [
                i for i, tRng in enumerate(p.tempRngLbl) if tRng == tempRng
            ]
            speedind = [
                i for i, sRng in enumerate(p.speedRngLbl) if sRng == speedRng
            ]
            mind = [i for i, mDet in enumerate(p.maxDets) if mDet == maxDets]
            cind = [i for i, cat in enumerate(p.catIds)
                    if cat == catId] if catId else range(len(p.catIds))
            if ap == 1:
            ...
            print(
                iStr.format(titleStr, typeStr, iouStr, catStr, areaRng,
                            tempRng, speedRng, maxDets, mean_s))
            return mean_s
        ...
        def _summarizeDets(catId=None):
            ...
            stats[1] = _summarize(
                1, iouThr=.3, maxDets=self.params.maxDets[2], catId=catId)
            stats[2] = _summarize(
                1, iouThr=.5, maxDets=self.params.maxDets[2], catId=catId)
            stats[3] = _summarize(
                1, iouThr=.75, maxDets=self.params.maxDets[2], catId=catId)
            stats[4] = _summarize(
                1,
                speedRng='fast',
