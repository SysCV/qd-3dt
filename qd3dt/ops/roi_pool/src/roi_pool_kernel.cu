...
__global__ void ROIPoolForward(const int nthreads, const scalar_t *bottom_data, const scalar_t *rois,
                               const scalar_t spatial_scale, const int channels, const int height, const int width,
                               const int pooled_h, const int pooled_w, scalar_t *top_data, int *argmax_data) {
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        scalar_t max_val = is_empty ? static_cast<scalar_t>(0) : bottom_data[bin_y1 * width + bin_x1] - 1;
...
int ROIPoolForwardLaucher(const at::Tensor features, const at::Tensor rois, const float spatial_scale,
                          const int channels, const int height, const int width, const int num_rois, const int pooled_h,
                          const int pooled_w, at::Tensor output, at::Tensor argmax) {
    const int output_size = num_rois * channels * pooled_h * pooled_w;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(features.type(), "ROIPoolLaucherForward", ([&] {
                                            ...
                                            ROIPoolForward<scalar_t><<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                                                output_size, bottom_data, rois_data, scalar_t(spatial_scale), channels,
                                                height, width, pooled_h, pooled_w, top_data, argmax_data);
                                        }));
...
template <typename scalar_t>
__global__ void ROIPoolBackward(const int nthreads, const scalar_t *top_diff, const scalar_t *rois,
                                const int *argmax_data, const scalar_t spatial_scale, const int channels,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        int bottom_index = argmax_data[(n * channels + c) * pooled_h * pooled_w + ph * pooled_w + pw];

        gpuAtomicAdd(bottom_diff + (roi_batch_ind * channels + c) * height * width + bottom_index, top_diff[index]);
    }
}

int ROIPoolBackwardLaucher(const at::Tensor top_grad, const at::Tensor rois, const at::Tensor argmax,
                           const float spatial_scale, const int batch_size, const int channels, const int height,
                           const int width, const int num_rois, const int pooled_h, const int pooled_w,
                           at::Tensor bottom_grad) {
    const int output_size = num_rois * pooled_h * pooled_w * channels;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.type(), "ROIPoolLaucherBackward", ([&] {
                                            ...
                                            ROIPoolBackward<scalar_t><<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                                                output_size, top_diff, rois_data, argmax_data, scalar_t(spatial_scale),
                                                channels, height, width, pooled_h, pooled_w, bottom_diff);
                                        }));
    AT_CUDA_CHECK(cudaGetLastError());
    return 1;
}
