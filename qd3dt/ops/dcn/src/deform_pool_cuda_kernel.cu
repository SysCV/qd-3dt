...
// modify from
// https://github.com/chengdazhi/Deformable-Convolution-V2-PyTorch/blob/mmdetection/mmdet/ops/dcn/src/cuda/deform_psroi_pooling_cuda.cu
...
__device__ scalar_t bilinear_interp(const scalar_t *data, const scalar_t x, const scalar_t y, const int width,
                                    const int height) {
    int x1 = floor(x);
    int x2 = ceil(x);
    int y1 = floor(y);
    ...
    scalar_t value = (1 - dist_x) * (1 - dist_y) * value11 + (1 - dist_x) * dist_y * value12 +
...
__global__ void DeformablePSROIPoolForwardKernel(
    const int count, const scalar_t *bottom_data, const scalar_t spatial_scale, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width, const scalar_t *bottom_rois,
    const scalar_t *bottom_trans, const int no_trans, const scalar_t trans_std, const int sample_per_part,
    const int output_dim, const int group_size, const int part_size, const int num_classes,
    const int channels_each_class, scalar_t *top_data, scalar_t *top_count) {
...
__global__ void DeformablePSROIPoolBackwardAccKernel(
    const int count, const scalar_t *top_diff, const scalar_t *top_count, const int num_rois,
    const scalar_t spatial_scale, const int channels, const int height, const int width, const int pooled_height,
    const int pooled_width, const int output_dim, scalar_t *bottom_data_diff, scalar_t *bottom_trans_diff,
    const scalar_t *bottom_data, const scalar_t *bottom_rois, const scalar_t *bottom_trans, const int no_trans,
    const scalar_t trans_std, const int sample_per_part, const int group_size, const int part_size,
        ...
        for (int ih = 0; ih < sample_per_part; ih++) {
            for (int iw = 0; iw < sample_per_part; iw++) {
                ...
                gpuAtomicAdd(
                    ...
                    diff_x);
                gpuAtomicAdd(bottom_trans_diff +
                                 (((n * num_classes + class_id) * 2 + 1) * part_size + part_h) * part_size + part_w,
                             diff_y);
            }
        }
    }
...
void DeformablePSROIPoolForward(const at::Tensor data, const at::Tensor bbox, const at::Tensor trans, at::Tensor out,
                                at::Tensor top_count, const int batch, const int channels, const int height,
                                const int width, const int num_bbox, const int channels_trans, const int no_trans,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.type(), "deformable_psroi_pool_forward", ([&] {
                                            ...
                                            DeformablePSROIPoolForwardKernel<<<GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
                                                ...
                                                (scalar_t)trans_std, sample_per_part, output_dim, group_size, part_size,
...
void DeformablePSROIPoolBackwardAcc(const at::Tensor out_grad, const at::Tensor data, const at::Tensor bbox,
                                    const at::Tensor trans, const at::Tensor top_count, at::Tensor in_grad,
                                    at::Tensor trans_grad, const int batch, const int channels, const int height,
                                    const int width, const int num_bbox, const int channels_trans, const int no_trans,
                                    const float spatial_scale, const int output_dim, const int group_size,
                                    const int pooled_size, const int part_size, const int sample_per_part,
                                    const float trans_std) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        out_grad.type(), "deformable_psroi_pool_backward_acc", ([&] {
            ...
            DeformablePSROIPoolBackwardAccKernel<<<GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
                ...
                pooled_height, pooled_width, output_dim, bottom_data_diff, bottom_trans_diff, bottom_data, bottom_rois,
                bottom_trans, no_trans, (scalar_t)trans_std, sample_per_part, group_size, part_size, num_classes,
                channels_each_class);
        }));
