...
def build_optimizer(model, optimizer_cfg):
    ...
    if paramwise_options is None:
        ...
        dcn_offset_lr_mult = paramwise_options.get('dcn_offset_lr_mult', 1.)
        embed_lr_mult = paramwise_options.get('embed_lr_mult', 1.)
        bboxfc_lr_mult = paramwise_options.get('bboxfc_lr_mult', 1.)
        ...
        for name, param in model.named_parameters():
            ...
            # otherwise use the global settings
                ...
                logger.info("{} with lr_multi: {}".format(
                    name, dcn_offset_lr_mult))
                param_group['lr'] = base_lr * dcn_offset_lr_mult
            elif name.find('embed_head') != -1:
                logger.info("{} with lr_multi: {}".format(name, embed_lr_mult))
        ...
        if isinstance(model.module, RPN):
            ...
            if issubclass(dataset_type, datasets.CocoDataset):
                if 'type' in eval_cfg.keys():
                    eval_type = eval_cfg.pop('type')
                    assert eval_type == 'voc'
                    runner.register_hook(
                        DistEvalmAPHook(val_dataset_cfg, **eval_cfg))
                else:
                    runner.register_hook(
                        CocoDistEvalmAPHook(val_dataset_cfg, **eval_cfg))
            elif issubclass(dataset_type, datasets.VideoDataset):
                if 'type' in eval_cfg.keys():
                    eval_type = eval_cfg.pop('type')
                    if eval_type == 'coco':
                        runner.register_hook(
                    ...
                    runner.register_hook(
                        VidDistEvalHook(val_dataset_cfg, **eval_cfg))
            else:
                runner.register_hook(
                    DistEvalmAPHook(val_dataset_cfg, **eval_cfg))
