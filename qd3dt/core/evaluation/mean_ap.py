...
def eval_map(det_results,
    ...
    if print_summary:
        print_map_summary(mean_ap, eval_results, dataset, novel_set)
...
def print_map_summary(mean_ap, results, dataset=None, novel_set=-1):
    ...
    for i in range(num_scales):
        ...
        if novel_set > -1:
            novel = NOVEL_CLASSES[novel_set]
            novel_ap = 0
            base_ap = 0
            for z, cat_name in enumerate(CLASSES):
                if cat_name in novel:
                    novel_ap += aps[i, z]
                else:
                    base_ap += aps[i, z]
            table_data.append([
                'Novel mAP', '', '', '', '',
                '{:.3f}'.format(novel_ap / len(novel))
            ])
            table_data.append([
                'Base mAP', '', '', '', '',
                '{:.3f}'.format(base_ap / (len(CLASSES) - len(novel)))
            ])
